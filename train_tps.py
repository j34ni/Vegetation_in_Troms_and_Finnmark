...
print('Epochs = ', epochs)
expname = 'TPS_' + str(Hlayers) + 'Hlayers-' + str(Olayers) + 'Olayers-' + str(epochs) + 'epochs-' + Hactivation + '-' + Oactivation
...
X_train_file = os.path.join(path, 'X_tps_train.hdf')
X_train = pd.read_hdf(X_train_file)

print('Reading X_test')
X_test_file = os.path.join(path, 'X_tps_test.hdf')
