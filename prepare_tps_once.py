...
## Troms og Finnmark
### Only mosses and lichens, and pixels with lichen at least once
...
## Use the mask to only keep pixels with lichen at least one year
...
## Save into local HDF5 file without index
    x_filename = os.path.join(path, 'x_tps_once_' + str(Year) + '.hdf')
    print(x_filename)
...
## Save into local HDF5 file without index
    y_filename = os.path.join(path, 'y_once_' + str(Year) + '.hdf')
    print(y_filename)
