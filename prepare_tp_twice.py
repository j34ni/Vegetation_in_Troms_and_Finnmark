...
## Troms og Finnmark
### Only mosses and lichens, and pixels with lichen at least twice
...
## Use a mask to only keep pixels with lichen at least two years
...
# Extract ERA5 t2m and tp fields
...
## Glue together df_t2m and df_tp and
...
## Save into local HDF5 file without index
    x_filename = os.path.join(path, 'x_tp_twice_' + str(Year) + '.hdf')
    print(x_filename)
...
## Save into local HDF5 file without index
    y_filename = os.path.join(path, 'y_twice_' + str(Year) + '.hdf')
    print(y_filename)
