...
# Onlt t2m
# Both the fractional cover, the ERA5 (2m temperature) values are normalized
...
# Normalizing temperature, total precipitation and snow depth values accordidng to these criteria
    ...
    df = ERA5_t2m.stack(z=['latitude', 'longitude']).to_pandas().transpose().reset_index()
...
# Create labels for ERA5-land variables to replace the dates
    ...
    df.set_axis(label_t2m, axis="columns", inplace=True)
...
## Save into local HDF5 file without index
    x_filename = os.path.join(path, 'x_t_' + str(Year) + '.hdf')
    print(x_filename)
